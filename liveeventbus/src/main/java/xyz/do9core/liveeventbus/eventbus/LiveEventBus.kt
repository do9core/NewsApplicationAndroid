...
abstract class LiveEventBus {
    ...
    interface Key {
         ...
         * Subject的默认索引
         * */
        companion object DefaultKey : Key
    }
     ...
     * */
    abstract fun <T : Event> subject(dataType: KClass<T>, key: Key = Key.DefaultKey)

    /**
     * 获取一个具有[dataType]类型和[key]索引的Subject的LiveData
     * */
    abstract fun <T : Event> get(dataType: KClass<T>, key: Key = Key.DefaultKey): SubjectLiveData<T>
    ...
    companion object : LiveEventBus() {
        ...
        override fun <T : Event> subject(dataType: KClass<T>, key: Key) {
            val keyPair = Pair(dataType, key)
            subjects.putIfAbsent(keyPair, SubjectLiveData<T>())
        }

        @Suppress("UNCHECKED_CAST")
        override fun <T : Event> get(dataType: KClass<T>, key: Key): SubjectLiveData<T> {
            val keyPair = Pair(dataType, key)
